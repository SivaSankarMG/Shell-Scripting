...
function service_status_check() {
    ...
    then
        ...
        echo "$1 is not active and running"
        exit 1
    fi
}
...
function firewalld_rule_check() {

    firewalld_ports=$(sudo firewall-cmd --list-all --zone=public | grep ports)

    if [[ $firewalld_ports == *$1* ]]
    then
        echo "$1 port is configured in Firewalld"
    else
        echo "$1 port is not configured in Firewalld"
        exit 1
    fi
...
service_status_check firewalld
...
firewalld_rule_check 80


# Update index.php
#command for red hat system [eg amazon linux, fedora, cent os]
sudo sed -i 's/index.html/index.php/g' /etc/httpd/conf/httpd.conf


#commands for debian systems [eg ubuntu]
#sudo sed -i 's/index.html/index.php/g' /etc/apache2/apache2.conf
#sudo sed -i 's/index.html/index.php/g' /etc/apache2/sites-available/000-default.conf
#echo 'DirectoryIndex index.php index.html' | sudo tee -a /etc/apache2/sites-available/000-default.conf
...
sudo git clone https://github.com/kodekloudhub/learning-app-ecommerce.git /tmp/ecommerce-app
sudo cp -r /tmp/ecommerce-app/* /var/www/html/
